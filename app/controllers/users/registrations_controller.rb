...
class Users::RegistrationsController < Devise::RegistrationsController
 ...
 Rails.logger.error"shyammmm from ruby #{resource.inspect} and #{resource.persisted?.inspect}"
 
 if resource.persisted?
   Rails.logger.error"shyammmm from ruby1"
  render json: {
    status: {code: 201 ,message: "User created Successfully",data: resource}
  }, status: :ok
 else
  Rails.logger.error"shyammmm from ruby2"
  render json:{
